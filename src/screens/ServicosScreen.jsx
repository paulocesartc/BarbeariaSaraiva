...
// Sheet bottom: safe area + padding fixo
...
export default function ServicosScreen() {
  ...
  async function handleCriar() {
    ...
    if (!name) {
      return showToast({ type: 'error', text1: 'Campo obrigatório', text2: 'Informe o nome do serviço.' });
    }
    if (!price || price <= 0) {
      return showToast({ type: 'error', text1: 'Preço inválido', text2: 'Informe um valor maior que zero.' });
    }
  ...
  function abrirEdicao(item) {
    ...
    setEditForm({
      name: item.name,
      // Converte o valor salvo para o formato mascarado (ex: 45.0 → "45,00")
  ...
  function abrirCombo() {
    if (selecionados.length < 2) {
      return showToast({ type: 'info', text1: 'Selecione mais serviços', text2: 'Escolha ao menos 2 para criar um combo.' });
    }
    ...
    return (
      ...
      >
        ...
        <View style={styles.iconContainer}>
          <MaterialCommunityIcons name={item.icon || 'content-cut'} size={24} color={colors.gold} />
        </View>
      ...
      {modoSelecao && (
        <View style={styles.barraCombo}>
          ...
          <View style={styles.barraAcoes}>
            ...
            <TouchableOpacity
              onPress={abrirCombo}
              style={[styles.btnBarra, styles.btnBarraCombo]}
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 8 }}>
                  ...
                  <TextInput
                    ...
                    value={form.price}
                    onChangeText={(v) => setForm((f) => ({ ...f, price: formatCurrencyInput(v) }))}
                  />
                </View>
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(v) => setForm((f) => ({ ...f, duration_min: v.replace(/\D/g, '') }))}
                  />
                </View>
              </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 20 }}>
                {ICONS.map((ic) => (
                  ...
                  >
                    <MaterialCommunityIcons name={ic} size={22} color={form.icon === ic ? colors.background : colors.gold} />
                  </TouchableOpacity>
                ))}
              </ScrollView>
              ...
              <View style={styles.modalBtns}>
                ...
                <TouchableOpacity onPress={handleCriar} style={styles.btnSalvar}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 8 }}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(v) => setEditForm((f) => ({ ...f, price: formatCurrencyInput(v) }))}
                  />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.label}>Duração (min)</Text>
                  <TextInput
                    ...
                    onChangeText={(v) => setEditForm((f) => ({ ...f, duration_min: v.replace(/\D/g, '') }))}
                  />
                </View>
              </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 20 }}>
                {ICONS.map((ic) => (
                  <TouchableOpacity
                    key={ic}
                    onPress={() => setEditForm((f) => ({ ...f, icon: ic }))}
                    style={[styles.iconBtn, editForm.icon === ic && styles.iconBtnActive]}
                  >
                    <MaterialCommunityIcons name={ic} size={22} color={editForm.icon === ic ? colors.background : colors.gold} />
                  </TouchableOpacity>
                ))}
              </ScrollView>
              ...
              <View style={styles.modalBtns}>
                ...
                <TouchableOpacity onPress={handleEditar} style={styles.btnSalvar}>
                  <Text style={styles.btnSalvarText}>Salvar</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          ...
          <KeyboardAvoidingView behavior="padding" keyboardVerticalOffset={0}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View style={styles.comboResumo}>
                {selecionados.map((s) => (
                  <View key={s.id} style={styles.comboItem}>
                    <MaterialCommunityIcons name={s.icon || 'content-cut'} size={16} color={colors.gold} />
              ...
              <TextInput
                ...
                keyboardType="number-pad"
                value={comboPreco}
                onChangeText={(v) => setComboPreco(formatCurrencyInput(v))}
              />
              ...
              {(() => {
                ...
                if (!isNaN(final) && desconto > 0) {
                  return (
                    <Text style={styles.descontoHint}>
                      Desconto de R$ {desconto.toFixed(2)} ({((desconto / somaSelecao) * 100).toFixed(0)}%)
                    </Text>
                  );
                }
                return null;
              ...
              <View style={[styles.modalBtns, { marginTop: 12 }]}>
                <TouchableOpacity onPress={() => setComboModal(false)} style={styles.btnCancelar}>
                  <Text style={styles.btnCancelarText}>Cancelar</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={handleCriarCombo} style={styles.btnSalvar}>
                  <Text style={styles.btnSalvarText}>Criar Combo</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
            </View>
